...
public sealed class FakeFormatCache : IFormatCache
{
    ...
    public Task<ThinkingFormat?> GetThinkingFormatAsync(string sourceUrl, string modelId, CancellationToken ct = default)
    {
        ...
        return Task.FromResult<ThinkingFormat?>(_thinking.TryGetValue(key, out var value) ? value : null);
    }
    ...
    {
        var key = Key(sourceUrl, modelId);
        return Task.FromResult<ToolCallFormat?>(_toolCalls.TryGetValue(key, out var value) ? value : null);
    }
