...
| **F-14** | **Context grants on server-pushed ops (Layer B, on by default)** | The bridge classifies every server-relayed request; a *Sensitive* one — provider-key spend (`/llm/proxy`), shell, the project file/git surface, MCP tool execution — is refused unless a node-signed grant covers the browser session. | A compromised server cannot silently spend your keys, browse your declared projects, run shell, or drive MCP tools: the grant is signed and checked **on the node**, and the server can relay bytes but cannot forge a signature. Enforcement is fail-closed. | The first sensitive op in a session shows an **in-chat approval prompt** (the node opens its local approval page); approving grants ~8 hours for that session, shared across chat, Explorer, `#` picker, and git. The toggle lives on the bridge status page (`// Security` tab) — never on the server. Joining a second node finishes with pasting the primary's master-key fingerprint on that machine (Soul panel → **JOIN · CONFIRM MASTER KEY**); until then, grants signed elsewhere are refused on the new node and Devices shows **JOIN NOT FINISHED**. | `RequestClassifier` (body-aware for `/tools/call`: read-only built-ins stay Benign); gate in `DirectTunnel.GateSensitiveAsync` on both the REST and streaming paths; grants signed with the soul/node key, stored in `ContextGrants`, re-verified on use, and mesh-replicated to sibling nodes (`/context/grants/export\|import` + 60 s background loop); ceremonies open on the node you pinned (`ApprovalNodePicker`). Joined nodes accept the soul public key only after a human-confirmed pin (`SoulKeyPinnedAt` / `SiblingRoster.ResolveSoulMasterPublicKey`); pin endpoints stay off `TunnelAllowlist`. |

**Software installs (`install_software`).** Package installs write to system locations outside any Allowed Path, so they cannot be honestly covered by the path-scoped Terminal policy above — they get a dedicated built-in with its own gating instead. The tool drives only a fixed manager allowlist (`brew`, `npm`, `pip`, `pipx`, `dotnet`, `cargo`, `go`, resolved by name on PATH), passes arguments directly to the process (no shell) after validating them against a strict charset, and runs the rendered command through the command blocklist as defense-in-depth. Server-side governance turns every call into an **in-chat approval** in Balanced, Coding, and Strict modes, blocks it outright in Plan mode, escalates it to a Seal in Paranoid mode — it runs unchecked only in **Off**, where nothing is enforced. Layer B classifies it Sensitive like shell.
...
| **Secondary nodes hold no soul private key** | Multi-node routing relies on the primary for soul-key operations (export, some seals). A joined node holds only its own node keypair plus a human-pinned copy of the soul *public* key (the last join step). Its local compromise does not expose the master private key, but it may expose its node key and synced data. |
