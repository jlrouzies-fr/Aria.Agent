...
| **F-4** | **Node-side Terminal policy + opt-in** | Commands run only if Terminal is enabled at the node, scoped to node-defined paths/blocklists. Project declarations also live on the bridge; the web displays them read-only. | A compromised server cannot run shell commands on your machine by default, cannot add project paths, and cannot bypass your path rules. An empty node-side Allowed Paths list blocks all paths rather than trusting the server. | You must enable **Terminal Capability** on the bridge status page. Set allowed paths and blocked commands there; the web Terminal tool modal only shows them with a link back to the bridge. | `TerminalEnabled` master switch, `TerminalAllowedPathsJson` / `TerminalBlockedCommandsJson`, `GET /terminal/projects`, `SecurityPolicy.FromNodeAndRequest` (empty node paths = block all; non-empty node paths: web may only narrow). |
