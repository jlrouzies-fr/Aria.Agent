...
- Runs local services: LLM proxy, Noosphere memory (optional built-in extract/embed models), MCP servers, terminal tools, and telemetry.
...
<table>
  <tr>
    ...
    <td valign="top">
      ...
      <p>The agent can <strong>Inscribe</strong> facts, <strong>Probe</strong> for recollections, and <strong>Contemplate</strong> across stored engrams to synthesise answers. Extraction and embeddings can run from <strong>opt-in built-in models</strong> on the bridge (no third-party inference engine required), from a channel you configure, or you can rely on full-text/graph recall without embeddings.</p>
      <p>Configure built-in models or extraction/embedding channels on the bridge's Memory tab, then browse stored engrams from the <code>// NOOSPHERE</code> sidebar in Aria.Web.</p>
    </td>
  </tr>
</table>
...
<p><strong>Verify nudge.</strong> After the agent edits files, the harness prompts it to check its work: while a turn accumulates successful file mutations (<code>write_file</code>/<code>edit_file</code>/<code>multi_edit</code>/deletes/moves) with no build or test run, the mutation's own result earns a one-line reminder — at the first mutation, then every five — pointing at <code>run_tests</code>. A passing <code>run_tests</code> or a <code>bash_exec</code> build/test command silences it for the rest of the turn. Advisory only: it never blocks a call and never counts against budgets (<code>Governance:VerifyNudge</code> config knob, on by default).</p>
...
- Coding tools: `grep`, git (`status`/`diff`/`log`/`stage`/`commit`/`discard`), `multi_edit`, `undo_file`, `/rewind` (whole-turn file rollback), persistent bash with background jobs (`process_list`/`process_output`/`process_kill`), `run_background`, `wait_for`, `system_info` environment recon, `http_request` API testing, `read_image` for vision models — all scoped to the node's Allowed Paths plus any `/scope` expansions
- Edit diff feedback: `edit_file`/`multi_edit`/`write_file` echo the unified diff back to the model (self-verify without a re-read), and approval cards show the prospective diff before you authorise (bridge `AgentTools:DiffFeedback` knob, on by default, 4 KB cap)
- Structured test feedback: `run_tests` infers the project's build/test command (or takes yours), maps a plain filter to the ecosystem's native flag, and returns pass/fail counts plus failing tests with file:line — dotnet/pytest/jest/vitest/cargo/go parsers, capped for context, same governance class as `bash_exec`
